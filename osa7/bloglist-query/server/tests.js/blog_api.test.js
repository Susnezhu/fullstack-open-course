...
describe("PUT requests", () => {
  test("put returns status 200", async () => {
    ...
    await api.put(`/api/blogs/${blogId}`).send({ likes: 15 }).expect(200);
  });
  ...
  test("amount likes changed", async () => {
    ...
    await api.put(`/api/blogs/${blogId}`).send(update).expect(200);

    const changedBlog = await api.get(`/api/blogs/${blogId}`).expect(200);
